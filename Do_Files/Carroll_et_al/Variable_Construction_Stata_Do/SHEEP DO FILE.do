use "D:\DATA\Data\STATA FILES\Ready for MASTERFILE\sheep\SHEEP 96-06 NOV 5.dta", clear
set more off
...
replace SHPopeninv=SHPopeninv/.787564 if t<=3 
replace SHPcloseinv=SHPcloseinv /.787564 if t<=3
replace SHPbreedopeninv=SHPbreedopeninv  /.787564 if t<=3
replace SHPbreedcloseinv=SHPbreedcloseinv /.787564 if t<=3
replace SHPtradeopeninv=SHPtradeopeninv /.787564 if t<=3
replace SHPtradecloseinv=SHPtradecloseinv /.787564 if t<=3
replace SHPwoolopeninv=SHPwoolopeninv /.787564 if t<=3
replace SHPwoolcloseinv=SHPwoolcloseinv /.787564 if t<=3
replace SHPsalesE=SHPsalesE /.787564 if t<=3
replace SHPpurchasesE=SHPpurchasesE /.787564 if t<=3
replace SHPfatsalesE=SHPfatsalesE /.787564 if t<=3
replace SHPstoresalesE=SHPstoresalesE /.787564 if t<=3
replace SHPstorepurE=SHPstorepurE /.787564 if t<=3
replace SHPbreedpurE =SHPbreedpurE/.787564 if t<=3
replace SHPbreedsalesE =SHPbreedsalesE/.787564 if t<=3
replace SHPhogsalesE=SHPhogsalesE /.787564 if t<=3
replace SHPwoolsalesE =SHPwoolsalesE /.787564 if t<=3
replace SHPbreedhogsalesE=SHPbreedhogsalesE /.787564 if t<=3
replace SHPcullsalesE =SHPcullsalesE/.787564 if t<=3
replace SHPbreedewesalesE=SHPbreedewesalesE /.787564 if t<=3
replace SHPhouseE =SHPhouseE/.787564 if t<=3
replace SHPtotalsubs =SHPtotalsubs /.787564 if t<=3
replace SHPconcentrates=SHPconcentrates /.787564 if t<=3
replace SHPwinfor=SHPwinfor /.787564 if t<=3
replace SHProots =SHProots /.787564 if t<=3
replace SHPpasture =SHPpasture/.787564 if t<=3
replace SHPmisc =SHPmisc/.787564 if t<=3
replace SHPtotalquota=SHPtotalquota/.787564 if t<=3
...
gen ALLOC1= SHPgrossoutput/cropandlivegrossoutput
replace ALLOC1=1 if ALLOC1>1
drop if ALLOC1<=0

/* Create Capital Input(end of year book values)= machinery+buildings */

generate C=ALLOC1*(((machinee/PTransportcap)*100)+((buildingse/POthercap)*100))

/* Create Land variables */

gen A1=SHPforagearea 
gen A2=SHPfeedarea
replace A1=A1*2.47105 if t==11
replace A2=A2*2.47105 if t==11
replace size3=size3*2.47105 if t==11

/* Create Direct costs */

gen D= (SHPconcentrates/PCattleFeed*100)+(SHPpasture/PTotalFert*100)+(SHPwinfor/PTotalFert*100)+(SHProots/PTotalInputs*100)+(ALLOC1*opexmachinery/PTotalInputs*100)+(ALLOC1*limee/PTotalFert*100)+ (ALLOC1*fuellubse/PMotorFuels*100)

/* Create Labour Inputs */

generate L3=ALLOC1*mandays
...
rename t T
rename w W 
rename SHPclass CLASS 
rename SHPclimate CLIMATE
rename SHPbreedopeninv BREEOP
rename SHPbreedcloseinv BREECL
rename SHPtradeopeninv TRADEOP
rename SHPtradecloseinv TRADECL
rename SHPwoolopeninv WOOLOP
rename SHPwoolcloseinv WOOLCL
rename SHPsalesE SALES
rename SHPpurchasesE PUR
rename SHPtotalsubs SUBS
rename SHPwoolsalesE WOOLSALE
rename size3 FSIZE
rename SHPtotalsheepaveQ YQ
rename SHPlivestockunitsQ LSU
rename SHPbreedpurE BREEPUR

gen HILLOW=0
replace HILLOW=1 if CLIMATE==1

gen	INTENSE=SHPQ/A2

keep BREEPUR INTENSE PVetExp AIALL SHPAI CTLQ SHPQ SHPgrossoutput SUBS LSU HILLOW DECOUP YQ FC T W CLASS CLIMATE BREEOP BREECL TRADEOP TRADECL WOOLOP WOOLCL SALES PUR WOOLSALE SYS FSIZE AGE PSheep  ALLOC1 C- SIZE5
rename PSheep PSHEEP


gen Y=(SALES/PSHEEP*100)+(WOOLSALE/PSHEEP*100)+(TRADECL/PSHEEP*100)-(WOOLOP/PSHEEP*100)+(WOOLCL/PSHEEP*100)-(TRADEOP/PSHEEP*100)
gen L=L3
gen A=A2

rename D DC
gen D=DC+(PUR/PSHEEP*100)

rename C CAP
gen C=CAP+(BREEOP/PSHEEP*100)+(BREEPUR/PSHEEP*100)
...
egen MY96=mean(Y) if T==1
egen Y96=max(MY96)
gen YT=Y/Y96

egen ML96=mean(L) if T==1
egen L96=max(ML96)
gen LT=L/L96

egen MA96=mean(A) if T==1
egen A96=max(MA96)
gen AT=A/A96

egen MC96=mean(C) if T==1
egen C96=max(MC96)
gen CT=C/Y96

egen MD96=mean(D) if T==1
egen D96=max(MD96)
gen DT=D/D96
...
gen SUBPC1=SUBS/SHPgrossoutput
gen SUBPC2=SUBS/Y
