...
* No Breeding Stock Dummy
gen NOBREED=0
replace NOBREED=1 if ctlopeninvbreed==0
replace NOBREED=1 if ctlcloseinvbreed==0
...
gen CTLAI=AIALL/PVetExp*100
generate AID=0
replace AID=1 if CTLAI>0
...
*********************************************************************
keep ctlsubs AGE CTLQ NOBREED DECOUP ALLOC1 SYS T W FC SCLASS FSIZE FVALUE AI ctlopeninv ctlcloseinv ctlopeninvbreed ctlcloseinvbreed ctlonpeninvtrade ctlcloseinvtrade ctlgrossoutput ctlpurchases ctlsalese ctlcalfpure ctlcalfsalese ctlweanerpure ctlweanersalese ctlstorepure ctlstoresalese ctlfinishsalese ctlbreedpure ctlbreedsalese ctlotherpure ctlothersalese PPrimeCattle PCowSlaughter PStoreCattle PTotalCattle CAP- SIZE5
...
gen ALLSALES=(CALFSALE/PSTORE*100) +(WEANSALE/PSTORE*100) +(STORSAL/PSTORE*100) +(FINSALE/PPRIME*100) +(BREESALE/PSALUT*100)+ (OTHSAL/PCATTLE*100)
drop if ALLSALES<=0

gen FINISHSALES=(STORSAL/PSTORE*100)+(FINSALE/PPRIME*100)
drop if FINISHSALES<=0

gen ALLOC2=(FINISHSALES)/ALLSALES
drop if ALLOC2<=.5

gen Y=(STORSAL/PSTORE*100)+(FINSALE/PPRIME*100)+(ALLOC2*TRADECL/PCATTLE*100)
drop if Y<=0

gen	H1=(ALLOC2*BREEDOP/PCATTLE*100)+(WEANPUR/PSTORE*100)+(STORPUR/PSTORE*100)+(ALLOC2*TRADEOP/PCATTLE*100)
gen	H2=(ALLOC2*BREEDOP/PCATTLE*100)+(ALLOC2*PUR/PCATTLE*100)+(ALLOC2*TRADEOP/PCATTLE*100)
gen 	H3=(WEANPUR/PSTORE*100)+(STORPUR/PSTORE*100)+(ALLOC2*TRADEOP/PCATTLE*100)
gen	L=L3*ALLOC2
gen   A=A1*ALLOC2
gen   C=CAP*ALLOC2
gen   D=D1*ALLOC2
...
********************************************************************************************
********* DATA CHAPTER: THE FOLLOWING ARE EMPLOYED BEFORE VARIABLES ARE DROPPED ************
********************************************************************************************
...
sum CALFSALE WEANSALE STORSAL FINSALE BREESALE OTHSAL
