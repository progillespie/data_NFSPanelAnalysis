*0 vectors
*DYconcentrates DYpasture DYwinfor opexmachinery 



*****************************
*** Setting quick_run = 1 enable QUICK RUN mode.
local quick_run = 1 
*****************************

local datadir G:\Data

capture mkdir `datadir'\data_FADNPanelAnalysis\OutData\DataCreatorFADN


if `quick_run' == 1{
	di "You're in QUICK RUN mode. Commands inside the following else{} statement will be ignored."
} 
else{
	di "You're in FULL RUN mode."
	cd `datadir'\data_FADNPanelAnalysis\Do_Files\FADN_IGM 
	use ../../OutData/FADN_IGM/fadn_igm, clear
	*** doFarmDerivedFADN calls renameFADN (maps varnames to NFS varnames) and tries
	*** to replicate what doFarmDerivedVars does for actual NFS data. It will leave a
	*** lot of variables as just == 0 for every observation, but some variables will be
	*** approximate matches.
	qui do sub_do\doFarmDerivedFADN.do
	cd "`datadir'\data_FADNPanelAnalysis\Do_Files\Carroll_et_al\Data_creator_Stata_do"
	save `datadir'\data_FADNPanelAnalysis\OutData\DataCreatorFADN\test_FADN_run.dta, replace
}



use G:\Data\data_FADNPanelAnalysis\OutData\DataCreatorFADN\test_FADN_run.dta, clear
...
rename fsizuaa size3 // originally fsizunad
rename flabtotl labunits 
rename flabsmds mandays
rename fainvfr farmvaluee
...
* THIS IS THE ORIGINAL KEEP STATEMENT
...
* THIS IS THE EDITED KEEP STATEMENT NEEDED TO GET THE FILE TO RUN TO COMPLETION
keep t year w* fc sys size3 labunits mandays farmvaluee machinee buildingse labcase conacree hiredlabe intereste opexmachinery opexbuildings upkeepe annuitiese ratee miscoverheade dairyinggrossoutput cattlegrossoutput sheepgrossoutput cropsgrossoutput cropandlivegrossoutput othergrantsandsubs aidairy aicattle aisheep age childunder5 childunder15 childunder19 loan sub? sub3? sub6? sub7? DYave DYmilkgal DYmilke DYsubs DYconcentrates DYpasture DYwinfor DYmisccosts DYforagearea DYfeedarea fuellubse fdferfil feedforgrazinglivestock feedforgrazinglivestockhomegrown changesinvalueoflivestock labfame teagasc



save `datadir'\data_FADNPanelAnalysis\OutData\DataCreatorFADN\test_data_creator_FADN.dta, replace

do "`datadir'\data_FADNPanelAnalysis\Do_Files\Carroll_et_al\Variable_Construction_Stata_Do\dairy_do_file.do"

* DATA FOR LIMDEP/NLOGIT. EXPLICITLY STATE EVERY VARIABLE NEEDED (SORTED ALPHA, NO RANGES, BUT WILDCARDS ARE ALLOWED)
keep AGE AI AID ALLOC C CHILD CHILD5 CHILD15 CHILD19 DC DECOUP EXTEN EXTEND FC FSIZE FVALUE H INTENSE LANDFAGE LANDFEED LANDRAT LOAN L3 CPI DEBTRAT SUBRAT1 SUBRAT2 SUBS1 SUBS2 SYS SIZE* T T? W year Y1 Y2 


outsheet using `datadir'\data_FADNPanelAnalysis\OutData\DataCreatorFADN\test_constructed_data_FADN.csv, comma replace

if `quick_run' == 1{
	di "You're in QUICK RUN mode. This is fine for intermediate runs, but you should run from the raw data prior to publishing results."
} 
else{
	di "You're in FULL RUN mode. If this is an intermediate run, and you'd like for the file to run quicker, then change quick_run to any value other than 1."
} 
