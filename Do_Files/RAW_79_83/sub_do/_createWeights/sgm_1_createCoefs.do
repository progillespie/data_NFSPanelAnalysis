* Generate Standard Gross margin or Standard Output for Farm Typology
* (i.e.  creating a system variable)
...
*
*   - These coefficients are obtainable from Eurostat (series ef_tsgm
*   and ef_tso for SGM and SO respectively)
*
*   - SGMs from before 1986 are available in the following three docs
*   from the Official Journal of the EU: + OJ L 148 1978 - CELEX:
*   31978D0463 + OJ L 128 1984 - CELEX: 31984D0542 + OJ C 191 1986 -
*   Google Search term "86/C 191/01" 
*
*   - The CELEX number is useful for searching on EUE-Lex.
*
*   - The SGMs from Eurostat and from the OJ have been combined into
*   consistent time series by Patrick R. Gillespie in the course of
*   his PhD thesis. The filename was SGMs_SOs_7307.xlsx, and it was
*   place in  Data/data_EUROSTAT/OutData/RAW_79_83.  NOTE: The names
*   of the coefficients changed over time and tables in the pdfs do
*   not properly cut and paste. If the spreadsheet is lost or
*   corrupted, you should expect to lose a day or so recreating it.
*
*   - In Stata 12 and above, it is possible to read directly from an
*   Excel spreadsheet, but to maintain compatability with older Stata
*   versions this code reads from a CSV created from that file. To
*   create the CSV correctly, filter the tables on the "Single Table"
*   sheet by the "Associated IB Varnames" column to hide blank cells
*   (those rows contain coefficients not used in Ireland). Then copy
*   and paste each table to a separate CSV, making sure to transpose
*   it (i.e. each column is a coefficient and each row is a year).
*
...
* is no meaningful production for a given category. We can safely set
* these to 0.
...
* Set up blank dataset (just key vars) to merge the coef's into
local maxyear = 2012 // the last year of the data


local  nobs = (`maxyear' - 1973 + 1) * 2
set obs `nobs' // 2 X number of years from 1973 to desired end year

gen     SGMregion = 1 if _n <= (`nobs'/2)
replace SGMregion = 2 if _n >  (`nobs'/2)
bysort SGMregion: egen year = seq() ,from(1973) to(`maxyear')
sort year
...
label save SGMregion using ///
  `dodir'/_value_labels/label_SGMregion, replace
...
*   intervening periods, it's appropriate to simply carry forward the
