...
* Recreate base data
scalar sc_recreate_data = 1

* Run Analysis
scalar sc_runanalysis = 0

* Type of Simulation

scalar sc_simulation = 1
scalar sc_orig_data = 0
...
* 2. Actual volume  change and price change for those without variable or farm attrition, but average change otherwise 
...
* 3. Actual price change for those without variable or farm attrition, but average change otherwise. No Volume Change
...
* 4. CSO price change for those without variable or farm attrition, but average change otherwise. No Volume Change
*scalar sc_volchange = 0 [no volume change]
...
* 4. Scenario price change for those without variable or farm attrition, but average change otherwise. No Volume Change
*scalar sc_volchange = 0 [no volume change]
...
* CHANGE-8412: Ensure that Output Data directories exist
...
* CHANGE-8412: Added vlist7a for deriving investment vars
...
* CHANGE-8412: Added bit for assigning weights. Optional run directly
*   in main file, will write standalone file as well.


***************************************************************
* UAA_WEIGHT (Assign weights to farms)
***************************************************************

* Weights tables have been calculated on an external spreadsheet. 
*   Weights must be assigned to farms as they fall into the cells of 
*   these tables. Do this and save in a separate dataset. This is 
*   basically a start on creating svy_weights.dta, so subsequent code
*   can recreate the other variables in that dataset.

do Cr_svy_weights.do "`OrigData'" "`OutData'"
