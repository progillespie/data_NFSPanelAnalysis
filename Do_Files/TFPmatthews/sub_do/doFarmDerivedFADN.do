...
/*----------------------------------------------------- 
P.Gillespie: generated in renameFADN.do
-----------------------------------------------------*/
*gen hasreps = fsubreps > 0 & fsubreps != .
*gen isreps = fsubreps > 0 & fsubreps !=.
		
/*----------------------------------------------------- 
P.Gillespie: might have these... set to 0 for now
-----------------------------------------------------*/
gen wave = year - 1982
...
/*----------------------------------------------------- 
P.Gillespie: Don't have these vars either
-----------------------------------------------------*/
...
/*----------------------------------------------------- 
P.Gillespie:  
I don't have ooinchild, isspofffarmy, oojobsps, and I 
 create teagasc in renameFADN.do, so turn all of this 
 off
...
/*----------------------------------------------------- 
P.Gillespie:  
Recreate using FADN system var, note the naming conflict (system is redefined as a categorical
var here)
-----------------------------------------------------*/
...
/*----------------------------------------------------- 
P.Gillespie: Can't gen (no man days variable)
-----------------------------------------------------*/
...
/*----------------------------------------------------- 
P.Gillespie: Recreate this using FADN regions 
-----------------------------------------------------*/
...
/*----------------------------------------------------- 
P.Gillespie: No soil codes in the FADN
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
...
Will have talk with Cathal about SFP. What's rule he's applying here?
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
* Single farm payment
gen sfp = 0
...
/*----------------------------------------------------- 
P.Gillespie: can I use subsidiesothercattle?
-----------------------------------------------------*/
...
/*----------------------------------------------------- 
P.Gillespie: Aggregated in FADN
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
*Suckler Welfare
...
/*----------------------------------------------------- 
P.Gillespie: sheepandgoats + othersubs?
-----------------------------------------------------*/
...
/*----------------------------------------------------- 
P.Gillespie: Then all this would be covered
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
...
/*----------------------------------------------------- 
P.Gillespie: totalsubsidies on crops
-----------------------------------------------------*/
...
/*----------------------------------------------------- 
P.Gillespie: possibly totalsubsidiesdairying ???
-----------------------------------------------------*/
gen dairy_subs = 0
...
/*----------------------------------------------------- 
P.Gillespie: don't have this... thinks it's already covered above anyway
-----------------------------------------------------*/
* Cow Protein Payment
...
/*----------------------------------------------------- 
P.Gillespie: FADN is more generic, so I think that we won't be able to do the following adjustments
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
...
/*----------------------------------------------------- 
P.Gillespie: presumably this will differ by country... have to work out how to do that
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
...
*land value
gen landval_ha = landval/fsizuaa //favlfrey was previous numerator
sort year 
...
/*----------------------------------------------------- 
P.Gillespie: This regression is nonsense until we sort out the variables in it (doesn't even run at the moment)
-----------------------------------------------------*/
...
gen tillage_area =  cerealsuaa + otherfieldcropsuaa 
/* Originally set to = wwhcuarq + swhcuarq + wbycuarq + sbycuarq + mbycuarq + wotcuarq + sotcuarq + osrcuarq + pbscuarq + lsdcuarq + potcuarq + sbecuarq */
...
gen flabpaid_lu = flabpaid/dpnolu
gen flabunpd_lu = flabunpd/dpnolu
...
/*----------------------------------------------------- 
P.Gillespie:
This drop statement eliminates all of the data (because
tfarmffi2 is always missing. Rather than fix it, I've just
turned it off as I don't understand what its purpose is.                 
-----------------------------------------------------*/
	*drop if tfarmffi2 == .
	drop if fulltime == .
...
/*----------------------------------------------------- 
P.Gillespie:
This section contains several changes to start and end
years. (Search "*!")

todo: The start and end years should certainly be passed 
      in by macros set in the master file, or possibly 
      by determining the min and max years in the data
      (although I don't know if the latter is appropriate)
-----------------------------------------------------*/
...
/*----------------------------------------------------- 
P.Gillespie: Several if statements (row selection) turned off (Search "*!") 
-----------------------------------------------------*/
sort soil
...
/*----------------------------------------------------- 
P.Gillespie: This loop is working with soil codes which FADN doesn't have. 
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
...
/*----------------------------------------------------- 
P.Gillespie: Again, soil codes are fake placeholder vars here
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
...
/*----------------------------------------------------- 
P.Gillespie: 
 Consulted Brian Moran. He confirmed dairy platform data is 
  collected by NFS, and measures "potentially available land" 
  for dairy expansion. There is no analogue to this in the FADN
  , and also no way to calculate it, so it must be excluded. 

 However, the code looks for several of these variables later
  on, so I simply code a placeholder version of 
  DAIRY_PLATFORM_HA to get it to complete.

 THESE VARIABLES ARE THEREFORE MEANINGLESS IN A FADN CONTEXT... 
  DO NOT USE THEM!
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
sort farmcode year
...
/*----------------------------------------------------- 
P.Gillespie: 
It is also pointless to tab this info (see comment above).
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
...
/*----------------------------------------------------- 
P.Gillespie: had to turn off several regressions here
-----------------------------------------------------*/
...
/*----------------------------------------------------- 
P.Gillespie: 
Turned off this entire section because several use 
 dairy platform vars which are only fake placeholders
 in this FADN version of the file. 
-----------------------------------------------------*/
/* ---------------  TURNED OFF ----------------------
...
/*----------------------------------------------------- 
P.Gillespie: 
Turned off because location doesn't exist on my system. 

todo: Change this hardcoded absolute filepath to a macro, 
      or at least to a relative filepath. 
-----------------------------------------------------*/
...
/*----------------------------------------------------- 
P.Gillespie:  
 As noted above, the dairy platform variables are meaningless and therefore should be dropped
   todo: any command using these variables should also be dropped
   todo: get this file to run without creating Dairy Platform vars in the first place.
-----------------------------------------------------*/
drop DAIRY_PLATFORM_HA spare_dry_platform exceed_dry_platform  hasexceed_dry_platform hasspare_dry_platform
