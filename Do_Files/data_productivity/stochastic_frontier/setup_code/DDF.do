...
*use "./9609v2.dta", clear

* See the spreadsheet called genPIndices to see new source data 
* from CSO and how I've calculated base changes as necessary. 
* Only a few indices relate to the Dairy model, so only those 
* indices have new values for t>11, all others simply inherit the
* t==11 value (although this shouldn't matter for the Dairy model anyway)
...
replace PMilk=111.4125351 if t==12
replace PMilk=112.3479888 if t==13
replace PMilk=78.20392891 if t==14
replace PMilk=100.5612722 if t==15
...
replace PTotalInputs=131.3 if t==12
replace PTotalInputs=155.9 if t==13
replace PTotalInputs=142.2 if t==14
replace PTotalInputs=139.9 if t==15
...
replace PCattleFeed=126.7 if t==12
replace PCattleFeed=147.4 if t==13
replace PCattleFeed=132 if t==14
replace PCattleFeed=122.9 if t==15
...
replace PTotalFert=136.4 if t==12
replace PTotalFert=220.6 if t==13
replace PTotalFert=185 if t==14
replace PTotalFert=162.2 if t==15
...
replace PMotorFuels=147.3 if t==12
replace PMotorFuels=173.6 if t==13
replace PMotorFuels=143.5 if t==14
replace PMotorFuels=168.6 if t==15
...
replace CPI=169.3 if t==12
replace CPI=176.1 if t==13
replace CPI=168.2 if t==14
replace CPI=166.7 if t==15
...
replace PTransportcap=110.4250909 if t==12
replace PTransportcap=114.2899691 if t==13
replace PTransportcap=114.518549 if t==14
replace PTransportcap=115.549216 if t==15
...
replace POthercap=105.7942182 if t==12
replace POthercap=107.2753373 if t==13
replace POthercap=106.7389606 if t==14
replace POthercap=103.6435307 if t==15
...
* PRG - Recreate specialisation variable per pg 88 of Final Report

gen SPECIAL = dairyinggrossoutput/farmgo
...
keep SPECIAL INTENSE DECOUP AI SYS T W FC SCLASS FSIZE FVALUE SEX AGE CHILD5 CHILD15 CHILD19 JOBTYPE CHILD5 CHILD15 CHILD19 LOAN Y1 QTOWN QTLEASE QTLET QT FAT PROTEIN LANDFAGE LANDFEED ALLOC- SIZE5
