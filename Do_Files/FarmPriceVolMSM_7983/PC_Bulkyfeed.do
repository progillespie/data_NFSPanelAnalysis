...
*Volume
qui replace v`var' = `var'_QTY

qui replace p`var' = `var'_EU / v`var'

* Calculate Mean Price, to impute price for those without a price
sort YE_AR BULKYFEED_CODE
by YE_AR BULKYFEED_CODE: egen tm`var' = mean(p`var') if p`var'  > 0 & p`var'  != .
replace tm`var' = 0 if tm`var' == .
by YE_AR BULKYFEED_CODE: egen m`var' = max(tm`var')	
replace p`var' = m`var' if p`var' == .

* Next years price
sort FARM_CODE BULKYFEED_CODE YE_AR
by FARM_CODE BULKYFEED_CODE: gen p`var'_p1 = p`var'[_n-1]
...
sort YE_AR BULKYFEED_CODE
by YE_AR BULKYFEED_CODE: egen tmdp`var' = mean(dp`var' ) if dp`var'  > 0 & dp`var'  != .
replace tmdp`var' = 0 if tmdp`var' == .
by YE_AR BULKYFEED_CODE: egen mdp`var' = max(tmdp`var')
replace dp`var' = mdp`var' if dp`var' == .

* Mean Volume, to impute price for those without a volume
sort YE_AR BULKYFEED_CODE
by YE_AR BULKYFEED_CODE: egen tmv`var' = mean(v`var') if v`var'  > 0 & v`var'  != .
replace tmv`var' = 0 if tmv`var' == .
by YE_AR BULKYFEED_CODE: egen mv`var' = max(tmv`var')
replace v`var' = mv`var' if v`var' == . | v`var' == 0

* Next years volume
sort FARM_CODE BULKYFEED_CODE YE_AR
by FARM_CODE BULKYFEED_CODE: gen v`var'_p1 = v`var'[_n-1]
...
sort YE_AR BULKYFEED_CODE
by YE_AR BULKYFEED_CODE: egen tmvd`var' = mean(dv`var') if dv`var'  > 0 & dv`var' != .
replace tmvd`var' = 0 if tmvd`var' == .
by YE_AR BULKYFEED_CODE: egen mvd`var' = max(tmvd`var')
